...
static C_BOOL aq_is_full(AQ_PERSON * pAq);
static C_BOOL aq_is_empty(AQ_PERSON * pAq);
static C_BOOL aq_extend(AQ_PERSON * pAq, int exlen);
static C_BOOL aq_enqueue(AQ_PERSON * pAq, int id, char * pname);
static C_BOOL aq_enqueue_bystruct(AQ_PERSON * pAq, PERSON_AQ * pnode);
static C_BOOL aq_dequeue(AQ_PERSON * pAq, PERSON_AQ * pnode);
static void aq_print(AQ_PERSON * pAq);
static void aq_print_all_id_status(AQ_PERSON * pAq);
static C_BOOL is_in_queue(AQ_PERSON * pAq, int index);
...
AQ_PERSON * aq_person_new(int arrlen){
	...
	pAq->is_inited = C_TRUE;
	return pAq;
}

//get the length of array queue
static int aq_len(AQ_PERSON * pAq){
	if (C_TRUE != pAq->is_inited){
...
static C_BOOL aq_enqueue(AQ_PERSON * pAq, int id, char * pname){
	...
	if (C_TRUE == pAq->is_full(pAq)){
		if(C_FALSE == pAq->extend(pAq, pAq->arrlen/2)){
			return C_FALSE;
		}
	}
...
static void aq_print_all_id_status(AQ_PERSON * pAq){
	...
	for (index=0; index < pAq->arrlen; index++){
		if (C_TRUE == is_in_queue(pAq, index)){
